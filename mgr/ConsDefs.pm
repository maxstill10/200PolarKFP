...
#$BUILD   = "#." . $STAR_HOST_SYS; print "build for $BUILD on host $host, STAR_OBJ = $STAR_OBJ, STAR_LIB = $STAR_LIB,STAR_BIN = $STAR_BIN\n" unless ($param::quiet);
 $BUILD   = "#" . File::Basename::basename(File::Basename::dirname($STAR_LIB)) ; print "build for $BUILD on host $host, STAR_OBJ = $STAR_OBJ, STAR_LIB = $STAR_LIB,STAR_BIN = $STAR_BIN\n" unless ($param::quiet);
