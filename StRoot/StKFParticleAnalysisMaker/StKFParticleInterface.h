...
class StKFParticleInterface: public TObject
{
 public:
  ...
  Char_t *GetOutFileNameMine() {return fOutFileName;} 
  Char_t *GetRunFileNameMine() {return fRunFileName;}
  //PID cuts 
  ...
  void SetOutFileNameMine(Char_t *inputName) {fOutFileName = inputName;} 
  void SetRunFileNameMine(Char_t *inputRun) {fRunFileName = inputRun;}
...
//  std::vector<int> GetPID(double m2, double p, int q, double dEdX, double dEdXPull[7], bool isTofm2, const int trackId);
  ...
  Char_t *fOutFileName, *fRunFileName;
